...
def view_participation(request, pk):
    user = get_object_or_404(User, pk=pk)
    sponsorships = sponsorship.objects.filter(participant=user)
    participations = participation.objects.filter(participant=user)
    return render(request, 'userApp/sponsorships.html', {'user': user,'participations':participations,'sponsorships'})
