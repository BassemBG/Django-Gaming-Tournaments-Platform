...
def face_recognition_video(request):
    ...
    # If a match is found, set the session flag and redirect on the next request
    if match_found:
        request.session['match_found'] = True
        return redirect(reverse_lazy('face_recognition_video'))

    # Check if the session has the match flag
    if 'match_found' in request.session and request.session['match_found']:
        del request.session['match_found']
